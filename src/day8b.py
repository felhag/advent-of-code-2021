...
def countOverlapWith(search, nmbr):
    if not nmbr:
        return -1

    return len(set(search) & set(nmbr))

# length, overlap with 4, overlap with 1
numberMapping = {
    0: [6, 3, 2],
    1: [2, None, None],
    2: [5, 2, None],
...
for line in input.splitlines():
    ...
    allWords = (end + ' ' + start).split(' ')
    mapping = {}

    while allWords:
        remaining = []
        for w in allWords:
            overlapWith4 = countOverlapWith(w, mapping.get(4))
            overlapWith1 = countOverlapWith(w, mapping.get(1))

            matched = False
            for key, stringnumber in numberMapping.items():
                if len(w) == stringnumber[0] and (stringnumber[1] is None or stringnumber[1] == overlapWith4) and (stringnumber[2] is None or stringnumber[2] == overlapWith1):
                    mapping[key] = w
                    matched = True
                    break
    ...
    for word in end.split(' '):
        for key, value in mapping.items():
            if value and set(value) == set(word):
                stringnumber += str(key)
                break
